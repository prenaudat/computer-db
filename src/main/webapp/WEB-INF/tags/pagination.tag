	...
	<div class="container text-center">
		<ul class="pagination">
			...
			<c:if test="${page.pageNumber>2}">
				<li><a
					href="${target}?page=${pageNumber-2}&query=${query}&order=${orderBy}&sort=${sort}&size=${size}">${pageNumber-2}</a></li>
			</c:if>
			<c:if test="${page.pageNumber>1}">
				<li><a
					href="${target}?page=${pageNumber-1}&query=${query}&order=${orderBy}&sort=${sort}&size=${size}">${pageNumber-1}</a></li>

			</c:if>


			<li><a
				href="${target}?page=${pageNumber}&query=${query}&order=${orderBy}&sort=${sort}&size=${size}">${pageNumber}</a></li>
		...
		<div class="btn-group btn-group-sm pull-right" role="group">
			<a
				href="${target}?size=10&query=${query}&order=${orderBy}&sort=${sort}"
				type="button" class="btn btn-default">10</a> <a
				href="${target}?size=50&query=${query}&order=${orderBy}&sort=${sort}"
				type="button" class="btn btn-default">50</a> <a
				href="${target}?size=100&query=${query}&order=${orderBy}&sort=${sort}"
